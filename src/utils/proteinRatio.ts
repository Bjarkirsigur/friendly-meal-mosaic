...
export const getProteinRatio = (calories: number, protein: number): {
  ...
  if (calories === 0) return { ratio: 0, classification: 'Low', color: 'text-rose-500' };
  
  const ratio = protein / calories;
  
  if (ratio >= 0.075) {
    return { ratio, classification: 'High', color: 'text-emerald-600' };
  } else if (ratio >= 0.04) {
    return { ratio, classification: 'Medium', color: 'text-amber-500' };
  } else {
    return { ratio, classification: 'Low', color: 'text-rose-500' };
  }
};
...
export const getProteinRatioDescription = (calories: number, protein: number): string => {
  ...
  return `${proteinPer100Cal}g protein per 100 calories`;
};
